...
resource "azurerm_application_insights_standard_web_test" "portal" {
  ...
  request {
    ...
    url = "https://${var.web_domains.portal}/"
  }
  validation_rules {
...
resource "azurerm_monitor_action_group" "dcop_tech" {
  name                = "pins-ag-dcop-tech-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "dcopDev" # needs to be under 12 characters
  tags                = local.tags
...
resource "azurerm_monitor_action_group" "dcop_service_manager" {
  name                = "pins-ag-dcop-service-manager-${var.environment}"
  resource_group_name = azurerm_resource_group.primary.name
  short_name          = "dcopDev" # needs to be under 12 characters
  tags                = local.tags
