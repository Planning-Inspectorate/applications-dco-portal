...
import { mockOtpCode } from '@pins/dco-portal-lib/testing/mock-otp.js';
...
describe('login controllers', () => {
	...
	describe('buildEnterOtpPage', () => {
		...
		it('should render enter otp page with errors if in view data', async () => {
			...
			const controller = buildEnterOtpPage();
			await controller(mockReq, mockRes, viewData);
	...
	describe('buildSubmitOtpController', () => {
		...
		it('should redirect back to enter otp page number of attempts exceeds 4', async (ctx) => {
			const now = new Date('2025-01-30T00:00:00.000Z');
			ctx.mock.timers.enable({ apis: ['Date'], now });
		...
		it('should redirect back to enter otp page number of attempts is equal to 4', async (ctx) => {
			const now = new Date('2025-01-30T00:00:00.000Z');
			ctx.mock.timers.enable({ apis: ['Date'], now });
