...
describe('statutory-nuisance-information save', () => {
	describe('buildSaveController', () => {
		it('should save statutory nuisance info journey successfully if hasStatutoryNuisanceStatement is yes', async () => {
			const mockDb = {
			...
			const mockRes = {
				...
				locals: {
					journeyResponse: {
						answers: {
							statutoryNuisanceStatement: 'doc-id-3,doc-id-2',
		...
		it('should save statutory nuisance info journey successfully if hasStatutoryNuisanceStatement is no', async () => {
			const mockDb = {
				$transaction: mock.fn((fn) => fn(mockDb)),
				case: {
					findUnique: mock.fn(() => ({
		...
		it('should throw error is issue encountered during db transaction to save case data', async () => {
			...
			await assert.rejects(() => controller(mockReq, mockRes), {
				message: 'error saving statutory nuisance information journey'
			});
		});
	});
});
