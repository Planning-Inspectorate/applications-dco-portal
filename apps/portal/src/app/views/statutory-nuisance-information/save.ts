		...
		try {
			await db.$transaction(async ($tx) => {
				...
				const categories: CategoryInformation[] = [
					{
						key: 'statutoryNuisanceStatement',
						subCategoryId: DOCUMENT_SUB_CATEGORY_ID.STATUTORY_NUISANCE_STATEMENT
					}
				];

				await deleteSubCategorySupportingEvidence($tx, caseId, categories);

				if (answers.hasStatutoryNuisanceStatement === BOOLEAN_OPTIONS.YES) {
			...
			logger.error({ error }, 'error saving statutory nuisance information data to database');
			throw new Error('error saving statutory nuisance information journey', { cause: error });
		}
